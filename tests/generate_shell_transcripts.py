...
def do_setup():
    ...
    config_file = config_file.replace('ad_location = false', 'ad_location = https://example.test/pub')
...
try:
    ...
    do_commands(shell, cmds=['mode sandbox'], name='mode_sandbox_alreadyonmode')
    do_commands(shell, cmds=['mode live'], name='mode_live')
    do_commands(shell, cmds=['mode live', 'mode sandbox'], name='mode_live_then_sandbox') # switch to live and then back to sandbox
    ...
    do_commands(shell, cmds=['hit extend ABC --assignments 1 --expiration 1'], name='hit_extend')
    ...
    do_commands(shell, cmds=['hit list --active --all-studies'], name='hit_list_active_allstudies')
    do_commands(shell, cmds=['hit list --reviewable'], name='hit_list_reviewable')
    ...
    do_commands(shell, cmds=['worker approve --all'], name='worker_approve_all')
    do_commands(shell, cmds=['worker approve --hit ABC'], name='worker_approve_hitid')
    do_commands(shell, cmds=['worker approve 123'], name='worker_approve_assignmentid')
    do_commands(shell, cmds=['worker approve --all --all-studies'], name='worker_approve_all_allstudies')
    
    #[]      worker reject (--hit <hit_id> | <assignment_id> ...) [--all-studies]
    do_commands(shell, cmds=['worker reject --hit ABC'], name='worker_reject_hitid')
    do_commands(shell, cmds=['worker reject 123 456'], name='worker_reject_assignmentids')
    do_commands(shell, cmds=['worker reject --hit ABC --all-studies'], name='worker_reject_hitid_allstudies')
    
    #[]      worker unreject (--hit <hit_id> | <assignment_id> ...) [--all-studies]
    do_commands(shell, cmds=['worker unreject --hit ABC'], name='worker_unreject_hitid')
    do_commands(shell, cmds=['worker unreject 123 456'], name='worker_unreject_assignmentids')
    do_commands(shell, cmds=['worker unreject --hit ABC --all-studies'], name='worker_unreject_hitid_allstudies')
    
    #[]      worker bonus  (--amount <amount> | --auto) (--reason) (--all | --hit <hit_id> | <assignment_id> ...) [--override-bonused-status] [--all-studies]
    do_commands(shell, cmds=['worker bonus --amount 1.00 --reason "Yee!" --all'], name='worker_bonus_amount_reason_all')
    do_commands(shell, cmds=['worker bonus --amount 1.00 --reason "Yee!" --hit ABC --override-bonused-status'], name='worker_bonus_amount_reason_hitid_override')
    do_commands(shell, cmds=['worker bonus --auto --reason "Yee!" --all'], name='worker_bonus_auto_reason_all')
    do_commands(shell, cmds=['worker bonus --amount 1.00 --reason "Yee!" --hit ABC'], name='worker_bonus_amount_reason_hitid')
    do_commands(shell, cmds=['worker bonus --amount 1.00 --reason "Yee!" 123 456'], name='worker_bonus_amount_reason_assignmentids')
    do_commands(shell, cmds=['worker bonus --amount 1.00 --reason "Yee!" --all --all-studies'], name='worker_bonus_amount_reason_all_allstudies')
    
    #[]      worker list [--submitted | --approved | --rejected] [(--hit <hit_id>)] [--all-studies]
    do_commands(shell, cmds=['worker list'], name='worker_list')
    do_commands(shell, cmds=['worker list --submitted'], name='worker_list_submitted')
    do_commands(shell, cmds=['worker list --approved'], name='worker_list_approved')
    do_commands(shell, cmds=['worker list --rejected'], name='worker_list_rejected')
    
    do_commands(shell, cmds=['worker list --hit ABC'], name='worker_list_hitid')
    do_commands(shell, cmds=['worker list --approved --hit ABC'], name='worker_list_approved_hitid')
    
    do_commands(shell, cmds=['worker list --submitted --all-studies'], name='worker_list_submitted_allstudies')
